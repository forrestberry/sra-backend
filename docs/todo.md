...
5. **Mastery + recommendations**: `update_math_fact_mastery` enforces the 95% accuracy & <2s latency rule (`training`→`mastered`→`needs_review`). Next step is building recommendation queries/endpoints (`GET /students/{id}/math-facts/mastery`, “missed twice” reports) that consume this data.
